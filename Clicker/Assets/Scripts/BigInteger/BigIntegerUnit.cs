...
public class BigIntegerUnit
{
	...
	{
		if (IsZero) return new char[]{'0'};

		StringBuilder sb = new StringBuilder();
		BigIntegerUnit tempValue = new BigIntegerUnit(this); // 값을 복사하여 변환 진행
		BigIntegerUnit ten = new BigIntegerUnit(10);
		bool isNegative = IsNegative;
    
		if (isNegative)
		{
			tempValue = -tempValue; // 양수로 변환 후 처리
		}

		while (!tempValue.IsZero)
		{
			BigIntegerUnit remainder;
			tempValue = Divide(tempValue, ten, out remainder); // 10으로 나누며 나머지를 구함
			sb.Insert(0, ToInt32(remainder)); // 나머지를 앞쪽에 추가
		}

		if (isNegative)
		{
			sb.Insert(0, "-"); // 음수인 경우 앞에 '-' 추가
		}
	...
	{
		StringBuilder sb = new StringBuilder();
		char[] charNumbers = ToDexString().ToCharArray();
